...
def send_borrow_data():
    ...
    student_id = normalize_student_id(request.form['student_id'])
    ...
    if not any(normalize_student_id(member["student_id"]) == student_id for member in club_members):
    ...
    if len(student_id) == 7:
        ...
        conn.execute(
            ...
            (club_id, student_id, student_name, key_number)
        )
        # 鍵の状態を更新する
        conn.execute(
